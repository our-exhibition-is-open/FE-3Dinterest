# 3Dinterest : 3D 모델 공유 서비스 (Pinterest Clone)
최근 웹 디자인 트렌드에서 3D 요소의 활용이 급증하고 있습니다. 3Dinterest는 3D 모델 파일(GLTF/GLB 등)을 렌더링하고 유저들과 공유할 수 있는 3D 전용 핀터레스트 클론 프로젝트입니다. 

### 프로젝트 개요
- 개발 기간: 2023.09 ~ 2023.12
- 개발 인원: 3인 교내 팀 프로젝트 (백엔드 2인, 프론트엔드 1인)
...
**3. 백엔드와 프론트엔드 개발 속도 차이에 따른 개발 지연 문제**
- 문제: 프론트엔드를 개발하기 위해선 백엔드의 api서버가 개발되기를 기다리거나, 더미데이터를 임시로 사용했다가 api연결을 위해 로직을 한 번더 수정해야하는 문제가 존재했음.
- 해결: msw모킹을 이용하여 실제 api연결 로직을 유지하여 프론트/백엔드의 개발 일정을 동시에 진행함.
---
